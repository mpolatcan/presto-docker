...
load_config "node-scheduler.max-splits-per-node" ${NODE_SCHEDULER_MAX_SPLITS_PER_NODE:=100} "config.properties"
load_config "node-scheduler.max-pending-splits-per-task" ${NODE_SCHEDULER_MAX_PENDING_SPLITS_PER_TASK:=10} "config.properties"
load_config "node-scheduler.min-candidates" ${NODE_SCHEDULER_MIN_CANDIDATES:=10} "config.properties"
load_config "node-scheduler.policy" ${NODE_SCHEDULER_POLICY:=uniform} "config.properties"
load_config "node-scheduler.network-topology.segments" ${NODE_SCHEDULER_NETWORK_TOPOLOGY_SEGMENTS:=NULL} "config.properties"
load_config "node-scheduler.network-topology.type" ${NODE_SCHEDULER_NETWORK_TOPOLOGY_TYPE:=NULL} "config.properties"
load_config "node-scheduler.network-topology.file" ${NODE_SCHEDULER_NETWORK_TOPOLOGY_FILE:=NULL} "config.properties"
load_config "node-scheduler.network-topology.refresh-period" ${NODE_SCHEDULER_NETWORK_TOPOLOGY_REFRESH_PERIOD:=NULL} "config.properties"
load_config "http-server.http.port" ${HTTP_SERVER_HTTP_PORT:=8080} "config.properties"
load_config "http-server.log.enabled" ${HTTP_SERVER_LOG_ENABLED:=true} "config.properties"
load_config "http-server.log.compression.enabled" ${HTTP_SERVER_LOG_COMPRESSION_ENABLED:=true} "config.properties"
load_config "http-server.log.path" ${HTTP_SERVER_LOG_PATH:=var/log/http-request.log} "config.properties"
load_config "http-server.log.max-history" ${HTTP_SERVER_LOG_MAX_HISTORY:=15} "config.properties"
load_config "http-server.log.max-size" ${HTTP_SERVER_LOG_MAX_SIZE:=NULL} "config.properties"
load_config "query.max-memory" ${QUERY_MAX_MEMORY:=50GB} "config.properties"
load_config "query.max-total-memory" ${QUERY_MAX_TOTAL_MEMORY:=NULL} "config.properties"
...
load_config "query.max-execution-time" ${QUERY_MAX_EXECUTION_TIME:=NULL} "config.properties"
load_config "query.max-run-time" ${QUERY_MAX_RUN_TIME:=NULL} "config.properties"
load_config "query.max-stage-count" ${QUERY_MAX_STAGE_COUNT:=NULL} "config.properties"
load_config "query.max-history" ${QUERY_MAX_HISTORY:=NULL} "config.properties"
load_config "query.min-expire-age" ${QUERY_MIN_EXPIRE_AGE:=NULL} "config.properties"
...
load_config "join-distribution-type" ${JOIN_DISTRIBUTION_TYPE:=AUTOMATIC} "config.properties"
load_config "redistribute-writes" ${REDISTRIBUTE_WRITES:=true} "config.properties"
load_config "memory.heap-headroom-per-node" ${MEMORY_HEAP_HEADROOM_PER_NODE:=NULL} "config.properties"
load_config "spill-enabled" ${SPILL_ENABLED:=false} "config.properties"
load_config "spill-order-by" ${SPILL_ORDER_BY:=true} "config.properties"
load_config "spill-window-operator" ${SPILL_WINDOW_OPERATOR:=true} "config.properties"
load_config "spiller-spill-path" ${SPILLER_SPILL_PATH:=NULL} "config.properties"
load_config "spiller-max-used-space-threshold" ${SPILLER_MAX_USED_SPACE_THRESHOLD:=0.9} "config.properties"
load_config "spiller-threads" ${SPILLER_THREADS:=4} "config.properties"
load_config "max-spill-per-node" ${MAX_SPILL_PER_NODE:=NULL} "config.properties"
load_config "query-max-spill-per-node" ${QUERY_MAX_SPILL_PER_NODE:=NULL} "config.properties"
load_config "aggregation-operator-unspill-memory-limit" ${AGGREGATION_OPERATOR_UNSPILL_MEMORY_LIMIT:=NULL} "config.properties"
load_config "spill-compression-enabled" ${SPILL_COMPRESSION_ENABLED:=false} "config.properties"
load_config "spill-encryption-enabled" ${SPILL_ENCRYPTION_ENABLED:=false} "config.properties"
load_config "exchange.client-threads" ${EXCHANGE_CLIENT_THREADS:=25} "config.properties"
load_config "exchange.concurrent-request-multiplier" ${EXCHANGE_CONCURRENT_REQUEST_MULTIPLIER:=3} "config.properties"
load_config "exchange.data-integrity-verification" ${EXCHANGE_DATA_INTEGRITY_VERIFICATION:=ABORT} "config.properties"
load_config "exchange.max-buffer-size" ${EXCHANGE_MAX_BUFFER_SIZE:=32MB} "config.properties"
load_config "exchange.max-response-size" ${EXCHANGE_MAX_RESPONSE_SIZE:=16MB} "config.properties"
load_config "sink.max-buffer-size" ${SINK_MAX_BUFFER_SIZE:=32MB} "config.properties"
load_config "task.concurrency" ${TASK_CONCURRENCY:=16} "config.properties"
load_config "task.http-response-threads" ${TASK_HTTP_RESPONSE_THREADS:=100} "config.properties"
load_config "task.http-timeout-threads" ${TASK_HTTP_TIMEOUT_THREADS:=3} "config.properties"
load_config "task.info-update-interval" ${TASK_INFO_UPDATE_INTERVAL:=3s} "config.properties"
load_config "task.max-partial-aggregation-memory" ${TASK_MAX_PARTIAL_AGGREGATION_MEMORY:=16MB} "config.properties"
load_config "task.max-worker-threads" ${TASK_MAX_WORKER_THREADS:=NULL} "config.properties"
load_config "task.min-drivers" ${TASK_MIN_DRIVERS:=NULL} "config.properties"
load_config "task.writer-count" ${TASK_WRITER_COUNT:=1} "config.properties"
load_config "scale-writers" ${SCALE_WRITERS:=false} "config.properties"
load_config "writer-min-size" ${WRITER_MIN_SIZE:=32MB} "config.properties"
load_config "optimizer.dictionary-aggregation" ${OPTIMIZER_DICTIONARY_AGGREGATION:=false} "config.properties"
load_config "optimizer.optimize-hash-generation" ${OPTIMIZER_OPTIMIZE_HASH_GENERATION:=true} "config.properties"
load_config "optimizer.optimize-metadata-queries" ${OPTIMIZER_OPTIMIZE_METADATA_QUERIES:=false} "config.properties"
load_config "optimizer.push-aggregation-through-outer-join" ${OPTIMIZER_PUSH_AGGREGATION_THROUGH_OUTER_JOIN:=true} "config.properties"
load_config "optimizer.push-table-write-through-union" ${OPTIMIZER_PUSH_TABLE_WRITE_THROUGH_UNION:=true} "config.properties"
load_config "optimizer.join-reordering-strategy" ${OPTIMIZER_JOIN_REORDERING_STRATEGY:=AUTOMATIC} "config.properties"
load_config "optimizer.max-reordered-joins" ${OPTIMIZER_MAX_REORDERED_JOINS:=NULL} "config.properties"
...
load_config "accumulo.zookeeper.metadata.root" ${ACCUMULO_ZOOKEEPER_METADATA_ROOT:=NULL} "catalog/accumulo.properties"
load_config "accumulo.cardinality.cache.size" ${ACCUMULO_CARDINALITY_CACHE_SIZE:=NULL} "catalog/accumulo.properties"
load_config "accumulo.cardinality.cache.expire.duration" ${ACCUMULO_CARDINALITY_CACHE_EXPIRE_DURATION:=NULL} "catalog/accumulo.properties"
# ===========================================================================
...
load_config "bigquery.views-enabled" ${BIGQUERY_VIEWS_ENABLED:=NULL} "catalog/bigquery.properties"
...
load_config "bigquery.max-read-rows-retries" ${BIGQUERY_MAX_READ_ROWS_RETRIES:=NULL} "catalog/bigquery.properties"
...
load_config "cassandra.load-policy.use-dc-aware" ${CASSANDRA_LOAD_POLICY_USE_DC_AWARE:=NULL} "catalog/cassandra.properties"
...
load_config "cassandra.load-policy.use-token-aware" ${CASSANDRA_LOAD_POLICY_USE_TOKEN_AWARE:=NULL} "catalog/cassandra.properties"
load_config "cassandra.load-policy.shuffle-replicas" ${CASSANDRA_LOAD_POLICY_SHUFFLE_REPLICAS:=NULL} "catalog/cassandra.properties"
...
load_config "cassandra.no-host-available-retry-timeout" ${CASSANDRA_NO_HOST_AVAILABLE_RETRY_TIMEOUT:=NULL} "catalog/cassandra.properties"
load_config "cassandra.speculative-execution.limit" ${CASSANDRA_SPECULATIVE_EXECUTION_LIMIT:=NULL} "catalog/cassandra.properties"
load_config "cassandra.speculative-execution.delay" ${CASSANDRA_SPECULATIVE_EXECUTION_DELAY:=NULL} "catalog/cassandra.properties"
load_config "cassandra.tls.enabled" ${CASSANDRA_TLS_ENABLED:=NULL} "catalog/cassandra.properties"
...
load_config "hive.recursive-directories" ${HIVE_RECURSIVE_DIRECTORIES:=NULL} "catalog/hive.properties"
load_config "hive.ignore-absent-partitions" ${HIVE_IGNORE_ABSENT_PARTITIONS:=NULL} "catalog/hive.properties"
load_config "hive.storage-format" ${HIVE_STORAGE_FORMAT:=NULL} "catalog/hive.properties"
load_config "hive.compression-codec" ${HIVE_COMPRESSION_CODEC:=NULL} "catalog/hive.properties"
load_config "hive.force-local-scheduling" ${HIVE_FORCE_LOCAL_SCHEDULING:=NULL} "catalog/hive.properties"
load_config "hive.respect-table-format" ${HIVE_RESPECT_TABLE_FORMAT:=NULL} "catalog/hive.properties"
load_config "hive.immutable-partitions" ${HIVE_IMMUTABLE_PARTITIONS:=NULL} "catalog/hive.properties"
load_config "hive.insert-existing-partitions-behavior" ${HIVE_INSERT_EXISTING_PARTITIONS_BEHAVIOR:=NULL} "catalog/hive.properties"
load_config "hive.create-empty-bucket-files" ${HIVE_CREATE_EMPTY_BUCKET_FILES:=NULL} "catalog/hive.properties"
load_config "hive.max-partitions-per-writers" ${HIVE_MAX_PARTITIONS_PER_WRITERS:=NULL} "catalog/hive.properties"
load_config "hive.nax-partitions-per-scan" ${HIVE_NAX_PARTITIONS_PER_SCAN:=NULL} "catalog/hive.properties"
load_config "hive.hdfs.authentication.type" ${HIVE_HDFS_AUTHENTICATION_TYPE:=NULL} "catalog/hive.properties"
load_config "hive.hdfs.impersonation.enabled" ${HIVE_HDFS_IMPERSONATION_ENABLED:=NULL} "catalog/hive.properties"
...
load_config "hive.non-managed-table-writes-enabled" ${HIVE_NON_MANAGED_TABLE_WRITES_ENABLED:=NULL} "catalog/hive.properties"
load_config "hive.non-managed-table-creates-enabled" ${HIVE_NON_MANAGED_TABLE_CREATES_ENABLED:=NULL} "catalog/hive.properties"
load_config "hive.collect-column-statistics-on-write" ${HIVE_COLLECT_COLUMN_STATISTICS_ON_WRITE:=NULL} "catalog/hive.properties"
load_config "hive.s3select-pushdown.enabled" ${HIVE_S3SELECT_PUSHDOWN_ENABLED:=NULL} "catalog/hive.properties"
load_config "hive.s3select-pushdown.max-connections" ${HIVE_S3SELECT_PUSHDOWN_MAX_CONNECTIONS:=NULL} "catalog/hive.properties"
load_config "hive.file-status-cache-tables" ${HIVE_FILE_STATUS_CACHE_TABLES:=NULL} "catalog/hive.properties"
load_config "hive.file-status-cache-size" ${HIVE_FILE_STATUS_CACHE_SIZE:=NULL} "catalog/hive.properties"
load_config "hive.file-status-cache-expire-time" ${HIVE_FILE_STATUS_CACHE_EXPIRE_TIME:=NULL} "catalog/hive.properties"
...
load_config "hive.orc.time-zone" ${HIVE_ORC_TIME_ZONE:=NULL} "catalog/hive.properties"
load_config "hive.timestamp-precision" ${HIVE_TIMESTAMP_PRECISION:=NULL} "catalog/hive.properties"
load_config "hive.temporary-staging-directory-enabled" ${HIVE_TEMPORARY_STAGING_DIRECTORY_ENABLED:=NULL} "catalog/hive.properties"
load_config "hive.temporary-staging-directory-path" ${HIVE_TEMPORARY_STAGING_DIRECTORY_PATH:=NULL} "catalog/hive.properties"
load_config "hive.metastore" ${HIVE_METASTORE:=NULL} "catalog/hive.properties"
...
load_config "hive.metastore.authentication.type" ${HIVE_METASTORE_AUTHENTICATION_TYPE:=NULL} "catalog/hive.properties"
...
load_config "hive.metastore.thrift.delegation-token.cache-ttl" ${HIVE_METASTORE_THRIFT_DELEGATION_TOKEN_CACHE_TTL:=NULL} "catalog/hive.properties"
load_config "hive.metastore.thrift.delegation-token.cache-maximum-size" ${HIVE_METASTORE_THRIFT_DELEGATION_TOKEN_CACHE_MAXIMUM_SIZE:=NULL} "catalog/hive.properties"
load_config "hive.metastore.thrift.client.ssl.enabled" ${HIVE_METASTORE_THRIFT_CLIENT_SSL_ENABLED:=NULL} "catalog/hive.properties"
...
load_config "hive.metastore.glue.pin-client-to-current-region" ${HIVE_METASTORE_GLUE_PIN_CLIENT_TO_CURRENT_REGION:=NULL} "catalog/hive.properties"
load_config "hive.metastore.glue.max-connections" ${HIVE_METASTORE_GLUE_MAX_CONNECTIONS:=NULL} "catalog/hive.properties"
load_config "hive.metastore.glue.max-error-retries" ${HIVE_METASTORE_GLUE_MAX_ERROR_RETRIES:=NULL} "catalog/hive.properties"
...
load_config "hive.metastore.glue.partitions-segments" ${HIVE_METASTORE_GLUE_PARTITIONS_SEGMENTS:=NULL} "catalog/hive.properties"
load_config "hive.metastore.glue.get-partition-threads" ${HIVE_METASTORE_GLUE_GET_PARTITION_THREADS:=NULL} "catalog/hive.properties"
load_config "hive.metastore.alluxio.master.address" ${HIVE_METASTORE_ALLUXIO_MASTER_ADDRESS:=NULL} "catalog/hive.properties"
load_config "hive.metastore-cache-ttl" ${HIVE_METASTORE_CACHE_TTL:=NULL} "catalog/hive.properties"
load_config "hive.metastore-cache-maximum-size" ${HIVE_METASTORE_CACHE_MAXIMUM_SIZE:=NULL} "catalog/hive.properties"
load_config "hive.metastore-refresh-interval" ${HIVE_METASTORE_REFRESH_INTERVAL:=NULL} "catalog/hive.properties"
load_config "hive.metastore-refresh-max-threads" ${HIVE_METASTORE_REFRESH_MAX_THREADS:=NULL} "catalog/hive.properties"
load_config "hive.metastore-timeout" ${HIVE_METASTORE_TIMEOUT:=NULL} "catalog/hive.properties"
...
load_config "hive.s3.storage-class" ${HIVE_S3_STORAGE_CLASS:=NULL} "catalog/hive.properties"
...
load_config "hive.s3.pin-client-to-current-region" ${HIVE_S3_PIN_CLIENT_TO_CURRENT_REGION:=NULL} "catalog/hive.properties"
load_config "hive.s3.ssl.enabled" ${HIVE_S3_SSL_ENABLED:=NULL} "catalog/hive.properties"
load_config "hive.s3.sse.enabled" ${HIVE_S3_SSE_ENABLED:=NULL} "catalog/hive.properties"
load_config "hive.s3.sse.type" ${HIVE_S3_SSE_TYPE:=NULL} "catalog/hive.properties"
...
load_config "hive.s3.upload-acl-type" ${HIVE_S3_UPLOAD_ACL_TYPE:=NULL} "catalog/hive.properties"
load_config "hive.s3.skip-glacier-objects" ${HIVE_S3_SKIP_GLACIER_OBJECTS:=NULL} "catalog/hive.properties"
...
load_config "hive.s3.max-error-retries" ${HIVE_S3_MAX_ERROR_RETRIES:=NULL} "catalog/hive.properties"
load_config "hive.s3.max-client-retries" ${HIVE_S3_MAX_CLIENT_RETRIES:=NULL} "catalog/hive.properties"
load_config "hive.s3.max-backoff-time" ${HIVE_S3_MAX_BACKOFF_TIME:=NULL} "catalog/hive.properties"
load_config "hive.s3.max-retry-time" ${HIVE_S3_MAX_RETRY_TIME:=NULL} "catalog/hive.properties"
load_config "hive.s3.connect-timeout" ${HIVE_S3_CONNECT_TIMEOUT:=NULL} "catalog/hive.properties"
load_config "hive.s3.socket-timeout" ${HIVE_S3_SOCKET_TIMEOUT:=NULL} "catalog/hive.properties"
load_config "hive.s3.max-connections" ${HIVE_S3_MAX_CONNECTIONS:=NULL} "catalog/hive.properties"
load_config "hive.s3.multipart.min-file-size" ${HIVE_S3_MULTIPART_MIN_FILE_SIZE:=NULL} "catalog/hive.properties"
load_config "hive.s3.multipart.min-part-size" ${HIVE_S3_MULTIPART_MIN_PART_SIZE:=NULL} "catalog/hive.properties"
...
load_config "hive.s3.cache.enabled" ${HIVE_S3_CACHE_ENABLED:=NULL} "catalog/hive.properties"
load_config "hive.s3.cache.location" ${HIVE_S3_CACHE_LOCATION:=NULL} "catalog/hive.properties"
load_config "hive.s3.cache.data-transfer-port" ${HIVE_S3_CACHE_DATA_TRANSFER_PORT:=NULL} "catalog/hive.properties"
load_config "hive.s3.cache.bookkeeper-port" ${HIVE_S3_CACHE_BOOKKEEPER_PORT:=NULL} "catalog/hive.properties"
load_config "hive.s3.cache.read-mode" ${HIVE_S3_CACHE_READ_MODE:=NULL} "catalog/hive.properties"
load_config "hive.s3.cache.ttl" ${HIVE_S3_CACHE_TTL:=NULL} "catalog/hive.properties"
load_config "hive.s3.cache.disk-usage-percentage" ${HIVE_S3_CACHE_DISK_USAGE_PERCENTAGE:=NULL} "catalog/hive.properties"
...
load_config "jmx.dump-period" ${JMX_DUMP_PERIOD:=NULL} "catalog/jmx.properties"
load_config "jmx.max-entries" ${JMX_MAX_ENTRIES:=NULL} "catalog/jmx.properties"
# ===========================================================================
...
load_config "kudu.client.default-admin-operation-timeout" ${KUDU_CLIENT_DEFAULT_ADMIN_OPERATION_TIMEOUT:=NULL} "catalog/kudu.properties"
load_config "kudu.client.default-operation-timeout" ${KUDU_CLIENT_DEFAULT_OPERATION_TIMEOUT:=NULL} "catalog/kudu.properties"
load_config "kudu.client.default-socket-read-timeout" ${KUDU_CLIENT_DEFAULT_SOCKET_READ_TIMEOUT:=NULL} "catalog/kudu.properties"
load_config "kudu.client.disable-statistics" ${KUDU_CLIENT_DISABLE_STATISTICS:=NULL} "catalog/kudu.properties"
load_config "kudu.schema-emulation.enabled" ${KUDU_SCHEMA_EMULATION_ENABLED:=NULL} "catalog/kudu.properties"
